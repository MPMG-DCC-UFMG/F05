...
path_to_dataset = os.path.join(path, "images")
path_to_data_file = os.path.join(path, "mpmg_annotations.csv")
...
def parse_one_annot(annotations_data, filename):
    class_dict = {'Classe':0, 'Classe ':0, 'Classe 1':0, 'Classe 2':1, 'Classe 3':2, 'Classe 4':3, 'Classe 5':4}
...
def test(test_loader, net, criterion, epoch, last):
    ...
    if last == True:
        ...
        torch.save(net.state_dict(), 'mpmg_classification_model'+args['version']+'.pth')
        print("model saved")

        # create a list containing the cases where the model fails
