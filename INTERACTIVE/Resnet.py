...
parser.add_argument('--pre_trained_imagenet', type=int, default=1)
...
class SIMECDataset(torch.utils.data.Dataset):
    def __init__(self, root, mode, transforms=None):
        ...
        with IO.open(os.path.join(root, "BALANCED_DATASETS", args.class_name, "{}_labels_treino.csv".format(args.class_name)), "r", encoding="UTF-8") as file:
        ...
        for row in self.data:
            ...
            for i in range(1,9):
                v.append(row[i])
            self.imgs = np.append(self.imgs, os.path.join(root, "IMAGES/" + k.split("-")[0] + "/" + k.split("-")[1] + ".jpeg"))
            self.dicio[k] = v
...
def get_classification_model():
    model = torchvision.models.resnet34(pretrained=args.pre_trained_imagenet)
